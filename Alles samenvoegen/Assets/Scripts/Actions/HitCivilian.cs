...
public class HitCivilian : MonoBehaviour {
    ...
    private void ShootGun() {
        Transform gunBarrel = this.transform.GetChild(1);
        Vector3 forward = gunBarrel.transform.TransformDirection(Vector3.forward);
        RaycastHit targetHit;

        Debug.DrawRay(gunBarrel.transform.position, forward, Color.red, 50);
        // Shoot the bullet, and if it hits, check if it is a civilian
        if (Physics.Raycast(gunBarrel.transform.position, forward, out targetHit)) {
